...
public class OrderDAO {
	...
	public void setOrder(String no, ArrayList<OrderDTO> orderArr, MemberDTO memberDto, String id, String date, String payment, String isPaid, int totPrice) {
		try {
			con = Config.getConnection();
			sql = "insert into poke.order(no, name, id, zip, add1, add2, tel, payment, totprice, ispaid, date, email) values(?,?,?,?,?,?,?,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
			...
			pstmt.setString(10, isPaid);
			pstmt.setString(11, date);
			pstmt.setString(12, memberDto.getEmail());
